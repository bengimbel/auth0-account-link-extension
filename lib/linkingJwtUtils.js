...
async function validateAuth0Token(childtoken) {
  ...
  try {
    ...
    console.log(`${JSON.stringify(decoded.payload)}: decoded.payload`);
    logger.info(`${JSON.stringify(decoded.payload)}: decoded.payload`);
    return decoded.payload;
  } catch (error) {
    console.log(`${JSON.stringify(error)}: error.jwt`);
    logger.info(`${JSON.stringify(error)}: error.jwt`);
