...
function fromHapi(serverFactory) {
  ...
  return async (context, req, res) => {
    ...
    console.log(`HELLO7: ${JSON.stringify(context)}`);
    console.log(`WHATAMI: ${JSON.stringify(context.clientMetadata)}`);
    if (hapiServer == null) {
      ...
      console.log(`HELLO5: ${JSON.stringify(req.x_wt)}`);
      console.log(`HELLO6: ${JSON.stringify(req.x_wt.ectx)}`);
...
function attachStorageHelpers(context) {
  console.log(`HELLO8: ${JSON.stringify(context.secrets)}`);
