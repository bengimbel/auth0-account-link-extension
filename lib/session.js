  ...
  async register(server, options) {
    ...
    server.route({
      ...
      handler: async (req, h) => {
        ...
        console.log(`idToken: ${idToken} is natural`);
        console.log(`idToken: ${JSON.stringify(idToken)}`);
        console.log(`${config('AUTH0_RTA')}/.well-known/jwks.json`);
        console.log(`${config('AUTH0_RTA')}/`);
        try {
          const getKey = await jwksRsa.hapiJwt2KeyAsync({
            cache: true,
            rateLimit: true,
          ...
          console.log(`getKey whatami: ${JSON.stringify(getKey)}`);
