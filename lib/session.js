...
module.exports = {
  ...
  async register(server, options) {
    console.log(`${JSON.stringify(options)} options session start`);
    logger.info(`${JSON.stringify(options)} options session start`);
    if (!options || typeof options !== 'object') {
      return new tools.ArgumentError('Must provide the options');
    }
    logger.info(`${JSON.stringify(options.onLoginSuccess)} options.onLoginSuccess session start`);
    ...
    server.route({
      ...
      handler: (req, h) => {
        ...
        console.log(`${JSON.stringify(options)}: options session login`);
    ...
    server.route({
      ...
      handler: async (req, h) => {
        ...
        try {
          ...
          console.log(`${JSON.stringify(decoded)}: decoded session login/callback`);
        ...
        console.log(`${JSON.stringify(req.state)}: req.state session login/callback`);
        ...
        try {
          ...
          console.log(`${JSON.stringify(token)}: token session login/callback`);
