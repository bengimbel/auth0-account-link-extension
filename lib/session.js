  ...
  async register(server, options) {
    ...
    server.route({
      ...
      handler: async (req, h) => {
        ...
        const accessToken = req.payload.access_token;
        console.log(`accessToken: ${JSON.stringify(accessToken)}`);
        console.log(`https://${config('AUTH0_RTA')}/.well-known/jwks.json`);
        console.log(`https://${config('AUTH0_RTA')}/`);
        try {
          const getKey = jwksRsa.hapiJwt2Key({
            cache: true,
            rateLimit: true,
            jwksRequestsPerMinute: process.env.NODE_ENV === 'test' ? 10 : 2,
            jwksUri: `https://${config('AUTH0_RTA')}/.well-known/jwks.json`
          });
