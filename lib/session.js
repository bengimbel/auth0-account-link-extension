  ...
  async register(server, options) {
    ...
    server.route({
      ...
      handler: (req, h) => {
        ...
        console.log('options session login');
        logger.info('options session login');
    ...
    server.route({
      ...
      handler: async (req, h) => {
        let decoded;
        console.log(`${JSON.stringify(req.payload)} req.payload  login/callback`);
        logger.info(`${JSON.stringify(req.payload)} req.payload  login/callback`);
        console.log(`${JSON.stringify(req.header)} req.body  login/callback`);
        logger.info(`${JSON.stringify(req.header)} req.body  login/callback`);
        const jwtVerifyAsync = promisify(jwt.verify);
        const idToken = req.payload.id_token;

        try {
          decoded = jwt.decode(idToken, { complete: true });
          console.log('decoded session login/callback');
          logger.info('decoded session login/callback');
        ...
        console.log('req.state session login/callback');
        logger.info('req.state session login/callback');
        if (
          (
        ...
        try {
          ...
          console.log('token session login/callback');
          logger.info('token session login/callback');
        } catch (error) {
          console.log(`Login callback failed: ${error}`);
          logger.info(`Login callback failed: ${error}`);
