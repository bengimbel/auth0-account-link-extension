    ...
    server.route({
      ...
      handler: async (req, h) => {
        ...
        try {
          ...
          console.log(`Invalid token from err: ${JSON.stringify(e)}`);
          return Boom.unauthorized('Invalid token from err');
        }

        if (!decoded) {
          console.log(`Invalid token is false: ${JSON.stringify(decoded)}`);
          return Boom.unauthorized('Invalid token is false');
        }
