...
const getSigningKey = promisify(jwksClient.getSigningKey).bind(jwksClient);
...
const verifyToken = async (token) => {
  ...
  const key = getSigningKey(kid);
  if (!key) {
    console.log(`Failed to get signing key for kid: ${kid}`);
  }
  const options = {
  ...
  const verifiedToken = await jwtVerifyAsync(token, key, options);
  if (!verifiedToken) {
    ...
    server.route({
      ...
      handler: async (req, h) => {
        ...
        try {
          ...
          // });
          // const getKey = jwksClient.hapiJwt2Key({
          //   cache: true,
          //   rateLimit: true,
          //   jwksRequestsPerMinute: process.env.NODE_ENV === 'test' ? 10 : 2,
          //   jwksUri: `${config('AUTH0_RTA')}/.well-known/jwks.json`
          ...
          // decoded = await jwtVerifyAsync(idToken, getKey, {
          ...
          console.log(`Token verification failed: ${JSON.stringify(e)}`);
          decoded = null;
        }
